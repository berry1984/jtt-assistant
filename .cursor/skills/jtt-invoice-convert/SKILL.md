...
- **美琦**：收件人信息按地址库编码从 `亚马逊仓库代码` sheet 查表；**海关编码保持源原值（不加小数点）**；报关方式含「退税」→ 一般贸易；渠道未映射时追加到 `服务渠道` 下拉；数据列 A-R，O=产品图片（源图嵌入）、P=PO Number（源 V 列）、Q=物品箱号（单行总箱数）、R=物品FBA ID（货箱编号 `U00000` 前 12 位）；可另传「订单列表 excel」（含「地址库编码」「运单号」列），按地址库编码回填 B1 客户订单号为运单号
