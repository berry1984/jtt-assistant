...
- **AB 备注** = `VAT:{VAT号}`，VAT号 按 **(运单号=系统SO号, FBA ID)** 匹配参考值「VAT号」列；
  匹配不到/该行为空 → 备注留空（查找表在丢弃 0 计费重行之前建，被丢行的 VAT 仍可用）
...
- **报价表A 样式**：数据行整行复制模板第 4 行的样式（字体/细边框/底纹/数字格式/行高 28.25），
  未用到的行按模板第 11 行复位——不要只清值，否则第 11 行起的新行没框线（格式不统一）
