...
def load_data(order_path, pick_path, price_path):
    ...
    prices = {}
    for r in ws.iter_rows(min_row=2, values_only=True):
        ch, wh, price, customs = r[0], r[1], r[2], r[3] if len(r) > 3 else None
        prices[wh] = {'channel': ch, 'price': price, 'customs_fee': customs or 0}
    
    return orders, picks, prices
        ...
        if so_picks:
            ...
            for fba, rows in so_picks.items():
                ...
                fba_rows.append({
                    'fba': fba,
                    'boxes': len(rows),
                    'length': rows[0][5], 'width': rows[0][4], 'height': rows[0][3],
                    'weight_raw': total_w,
                })

            for r in fba_rows:
                r['weight'] = round(r['weight_raw'])
            ...
            for r in fba_rows:
                ...
                r.update({'so': so, 'service': service, 'date': date_val,
                          'wh': wh_code, 'unit_price': p.get('price', 0) if p else 0})
            ...
            all_rows.append({
                ...
                'boxes': o['件数'] or 0, 'length': 0, 'width': 0, 'height': 0,
                'weight': total_weight_order, 'weight_raw': total_weight_order,
                'unit_price': p.get('price', 0) if p else 0,
            })
...
def generate_bill(rows, output_path, template_path=None, title_str=None, date_range_str=None):
    ...
    for ch, s_row, e_row in channel_groups:
        ...
        # Add borders and formatting to merged cells
    ...
    # ── 报价表A: update week info ──
    ws_quote = wb['报价表A']
    if date_range_str:
        parts = date_range_str.split('.')
        month = int(parts[0])
        start_day = int(parts[1].split('-')[0])
        # Compute week of month: first Mon of month = start of week 2
        from datetime import date as dt_date
        monday_dt = dt_date(2026, month, start_day)
        first_of_month = dt_date(2026, month, 1)
        days_to_monday = (7 - first_of_month.weekday()) % 7
        first_monday = first_of_month.replace(day=1) + timedelta(days=days_to_monday) if days_to_monday else first_of_month
        ...
        for r in range(4, ws_quote.max_row + 1):
            if ws_quote.cell(row=r, column=1).value is not None:
                ws_quote.cell(row=r, column=1).value = int(month)
                ws_quote.cell(row=r, column=2).value = week_str
...
def main():
    ...
    # Load
    orders, picks, prices = load_data(order_path, pick_path, price_path)
    ...
    # Generate bill
    success = generate_bill(rows, output_path, title_str=title_str, date_range_str=date_range_str)
