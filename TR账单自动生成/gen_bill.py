...
规则（2026-09-21 改：与参考值**严格一一对应**，不再做任何推断匹配）：
  - 行序：账单行 = 参考值表从上到下的行序，一一对应；不按 SO 归并、不重排
  - 发货日期(A) = 下单时间   SO(B) = 系统SO号   FBA(C) = FBA ID
    走货渠道(D) = 客户渠道   仓库代码(E) = 仓库代码   箱数(F) = 总箱数(CTN)
...
def load_data(ref_path):
    ...
    (ref_rows, price_rows_raw, declaration_groups)"""
    ref_rows, price_rows_raw = _parse_reference(ref_path)
    # ── 报关分组规则：在生成账单时按账单内每行的 (走货渠道, SO) 直接判定（见 generate_bill） ──
    declaration_groups = []
    return ref_rows, price_rows_raw, declaration_groups

def compute_ref_weight(row, channel):
    """复刻参考值模版「计费重」列公式：ROUND(MAX(参考实重×箱数, 参考材积重×箱数), 0)。

    仅在参考值「计费重」列读不到数值时作为兜底（见 charge_weight_of）：
    拣货模块用 openpyxl 写公式不带缓存值，data_only 读出来是 None；用户手工改过或
    在 Excel/WPS 里存过的文件则有数值，那种情况直接取列值。
    参考实重/参考材积重（W/X/Y/Z 列，来自箱规历史）缺失时回退实际尺寸（O/P/Q/R）。
...
def charge_weight_of(ref):
    """账单 J 列计费重：直接取参考值「计费重」列的数值。

    该列在拣货模块写出的文件里是公式（openpyxl 不带缓存值，data_only 读出 None），
    此时按模版公式复刻计算（compute_ref_weight）；用户在 Excel/WPS 里存过、
    或手工改过该列的文件则直接取列值。
    """
    v = ref.get('charge_w')
    if isinstance(v, (int, float)):
    ...
    if isinstance(v, str):
        ...
        if s and not s.startswith('='):
            try:
                ...
                pass
    return compute_ref_weight(ref, ref.get('channel') or '')


def build_rows(ref_rows):
    """把参考值行**一一映射**为账单行，严格保持参考值表从上到下的行序。

    账单每列都直接取自参考值同名列，不做任何推断/回退匹配：

      A 发货日期 ← 下单时间        B SO ← 系统SO号        C FBA ← FBA ID
    ...
    dropped = []
    for ref in ref_rows:
        ref_weight = charge_weight_of(ref)
        if ref_weight <= 0:
            dropped.append(str(ref.get('so') or '(无SO)'))
            continue
        e_price = ref.get('e_price')
        rows.append({
            'so': ref['so'],
            'fba': ref['fba'],
            # 发货日期 = 该行所属组的「下单时间」（8月19日格式不变，解析不了留空）
            'date': parse_order_date(ref.get('order_time')) or '',
            'service': (ref.get('channel') or '').strip(),
            'wh': ref.get('wh') or '',
            'boxes': int(to_num(ref.get('boxes'))),
            # 长宽高取「参考」尺寸（参考长/参考宽/参考高），不是发票实际尺寸
            'length': to_num(ref.get('ref_l')),
            'width': to_num(ref.get('ref_wid')),
    ...
    if dropped:
        ...
        print(f"  ⚠️  计费重为 0 已丢弃 {len(dropped)} 行（报关费按行收，故不留）：{shown}")
    return rows
...
def main():
    ...
    n_ref = len(ref_rows)
    n_so = len({r['so'] for r in ref_rows})
    print(f"✅ 参考值: {n_ref} 行（{n_so} 个SO）, 报价表A条目: {len(price_rows_raw)} 条")
    ...
    sum_O = sum(r['weight'] * _price(r) * 0.07/1.06 for r in rows)
    sum_P = sum_O * 0.06
    sum_Q = sum(r['weight'] * _price(r) * 0.35 for r in rows)
    sum_R = sum(r['weight'] * _price(r) * 0.58 for r in rows)
